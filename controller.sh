...
#######################################################
#   Note: It's possible to have this script
#       autogenerate a new room every time it starts
#       instead of always going to 4556. Simply remove
#       the "room": '"${ROOM}"', JSON from the 
#       "Create New AudioBridge room" request (about
#       15 lines down), and uncomment the section
#       following it where ROOM is set to ROOM_ID
#       
#       You can then find the autogenerated room id
#       in the logs of this script or have them 
#       emailed to yourself
######################################################
...
gst-launch-1.0 rpicamsrc ! video/x-raw,width=640,height=480 ! x264enc speed-preset=ultrafast tune=zerolatency byte-stream=true bitrate=400 threads=1 ! h264parse config-interval=1 ! rtph264pay pt=96 ! udpsink host="${IP}" port=8004 alsasrc device=plughw:1,0 ! audioconvert ! audioresample ! opusenc ! rtpopuspay pt=10 ! udpsink host="${IP}" port=8005
# srtp encrypt?
